...
For standalone playback, pass `narrationReady={narration.isReady}` alongside `onSceneChange={narration.onSceneChange}`, and call `narration.interrupt()` from the player's `onError` handler to stop pending audio; `VideoChat` wires this automatically. For grouped paragraphs, the voice must invoke `onStart` when audio actually begins, not when audio is prepared or `play()` is requested. The first visual cue starts narration, then the playhead waits for that onset without pausing the voice. Completion, failure, or interruption releases the readiness latch; a missing onset stops the player with an error after eight seconds of active waiting. This aligns the start of each paragraph; it is not a continuously audio-driven clock.
