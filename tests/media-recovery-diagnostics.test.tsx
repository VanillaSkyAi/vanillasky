...
it.each(["decode-error", "frame-readiness-timeout", "stalled-media"])("reports only a safe reason for actual %s recovery", async reason => {
  ...
  const view = render(<MountedReadinessContext.Provider value={vi.fn()}><VideoFrame config={config} time={1} width={360} height={640} playing /></MountedReadinessContext.Provider>);
