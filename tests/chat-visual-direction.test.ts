...
describe('answer visual direction', () => {
  it.each(['explanation', 'practical', 'story', 'comedy', 'imagination'])('keeps %s intent while defaulting missing or invalid style to realistic', intent => {
    for (const visualStyle of [undefined, null, '', 'unknown', 'cinematic', 'constructor', {}, 1]) {
  ...
  it.each(['illustrated', 'realistic'])('accepts an explicitly planned %s style for any intent', visualStyle => {
  ...
  it.each([undefined, 'constructor', {}, 'comparison'])('validates intent independently from a valid planned style (%s)', intent => {
    expect(compileVisualDirection({intent, visualStyle: 'illustrated'})).toMatchObject({intent: 'explanation', visualStyle: 'illustrated'});
  });
