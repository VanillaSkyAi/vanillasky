...
test("hovered floating captions own clicks while the idle attribute catches up", async ({ page }) => {
    ...
    <div class="vanillasky-video-chat" style="height:900px">
      ...
      <div class="panel"><div class="panel-inner"><div class="caption-slot" data-captions="true"><div class="caption-clip">
        <div class="line-row" data-actions-visible="false"><p class="line">The complete subtitle stays readable.</p>
          <div class="caption-actions"><button class="caption-action" onclick="this.textContent='Expanded'">Expand subtitles</button></div>
        </div>
      </div></div></div></div>
    </div>`);
  await page.locator(".line-row").hover();
  const button = page.getByRole("button", { name: "Expand subtitles" });
  ...
  await button.click();
  await expect(page.getByRole("button", { name: "Expanded" })).toBeVisible();
});
