...
const video:Video={schemaVersion:VIDEO_SCHEMA_VERSION,orientation:portrait?'portrait':'landscape',style:{},scenes:[{id,templateId:id,variables:{...examples[id],...(["photo","video"].includes(params.get("background")??"")?{mediaUrl:`/tests/browser/fixtures/media-transition/waterfall.${params.get("background")==="video"?"mp4":"jpg"}`,mediaType:params.get("background"),mediaPoster:"/tests/browser/fixtures/media-transition/waterfall.jpg"}:{})},timing:{fixedDuration:8}}]};
