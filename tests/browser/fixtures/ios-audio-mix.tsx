...
const proof = {
  ...
  nativeAudio: 0, nativeInterruptions: 0, contexts: 0, frames: 0,
...
function deliverBody() {
  ...
  const scene = { id: "water", templateId: "cinemaMedia", variables: {
    mediaUrl: new URL(fixture(new URLSearchParams(location.search).has("webm") ? "waterfall-audio.webm" : "waterfall-audio.mp4"), location.href).href,
    mediaType: "video", mediaAudio: "ambient", fallbackText: "Water keeps moving",
  }, narration: body, timing: { fixedDuration: 5 } };
  const snapshot: Video = { schemaVersion: "0.2", orientation: "portrait", style: {}, audio, scenes: [scene] };
