# Bounded mobile preparation experiment

This isolated experiment starts from SDK 0.10.7 (`825a112`). It is not a
physical-device safety result or a release candidate. The internal
`BOUNDED_MOBILE_PREPARATION` switch in `video-frame.tsx` enables it without a
public API change.
...
Run the bounded offline experiment:
...
Before default adoption: measure repeated cuts, cancellation, memory pressure,
and background/foreground behavior on real iPhone/iPad hardware at production
media resolutions. Desktop WebKit with an iPhone user agent cannot establish
mobile decoder memory safety. Existing tests requiring exactly one retained
mobile decoder are intentionally not rewritten in this experimental branch;
that production contract needs an explicit decision after physical evidence.
