...
type Event = { kind: string; at: number; id?: number; name?: string; code?: number; source?: string };
const events: Event[] = [];
...
HTMLMediaElement.prototype.play = function () {
  if (!identities.has(this)) {
    ...
    for (const kind of ['playing', 'ended', 'error']) this.addEventListener(kind, () => record({ kind, id: identities.get(this), code: this.error?.code }));
  }
