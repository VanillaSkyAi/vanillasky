...
function App() {
  ...
  const [ducked, setDucked] = useState(false), [waiting, setWaiting] = useState(false), [music, setMusic] = useState(0), [voiceVolume, setVoiceVolume] = useState(1);
  const voice = useRef<ReturnType<typeof createVideoChatVoice> | undefined>(undefined);
  const start = () => {
    voice.current ??= createVideoChatVoice({ fetcher: () => fetch(fixture('paragraph.mp3')), onActivityChange: value => { events.push(value ? 'audible' : 'quiet'); setDucked(value); }, onFallback: () => events.push('fallback') });
  ...
  return <>
    ...
    {started && <VideoPlayerRuntime video={video} controls={false} width={240} startMuted={false} muted={muted} paused={paused} soundtrack={track(music)} soundtrackVolume={.2} backgroundDucked={ducked} backgroundWaiting={waiting} nativeMediaAudio={{ volume: .15, ambientOnly: true }} />}
  </>;
}
