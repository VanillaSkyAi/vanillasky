...
export function createVideoChatResponseInstructions(
  ...
  return [
    ...
    `Saved ending: {"type":"ending","title":"short meaningful title","narration":"complete final payoff","subject":"literal subject","action":"visible action and useful framing","durationSec":${duration},"continuity":"cut|continue"}. It follows the same field bounds as a shot, is stored for playback last and is never repeated as a shot.`,
    `Plan a complete answer within at most ${scenes} scenes INCLUDING the saved ending, plus the opening. ${scenes === 1 ? 'Emit the compact answer brief with empty development, then an ending containing the complete answer; emit no shots.' : 'Emit the compact answer brief, then the first developing shot immediately so its footage can start. Next emit the saved ending, then any remaining developing shots in narrative order. Stop after the saved ending and all developing shots have been emitted. Do not delay the first shot to write later scenes.'} Do not put an ending inside the brief.`,
    generated
      ? `Each narration, including the ending, has ${budget.maxSpeechSec} seconds of speech: at most ${budget.targetWords} ordinary words or ${budget.targetUnspacedCharacters} characters in languages without spaces. Mixed scripts share this budget. Budget numbers, units and abbreviations as spoken, not compact notation; count before emitting.`
    ...
    ...(mode === "cinematic" ? [
      `AI VIDEO: describe one coherent generated shot per beat. Keep recurring subjects, appearance and setting consistent. ${generatedVideoAudio ? "Only subtle environmental ambience and action sounds matching the visible scene. Never include voices, speech, dialogue, narration, singing, chanting or music." : "Generated footage is silent."} Footage has no written text: do not ask subjects to speak or show captions or headline cards.`,
