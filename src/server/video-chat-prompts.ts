...
export function createVideoChatResponseInstructions(
  ...
  return [
    ...
    "Use one shared photorealistic, cinematic treatment with natural materials, believable proportions, motivated lighting and realistic depth. This applies to people, animals, objects and places, including hypothetical or fictional situations. When a hidden structure, relationship or process needs explanation, describe a cutaway, transparent layers or simplified geometry in that shot's action while retaining realistic materials and lighting. Keep the mechanism and spatial relationships clear; do not switch to flat artwork just because the topic is scientific or a diagram is needed. Use explanatory views only when they help understanding, then return to the relevant observable subject or result. Preserve an explicitly user-requested aesthetic in shared visualDirection when it applies to the whole answer, or only in the relevant shot action when its scope is local; identify it as user-requested and never invent an aesthetic override from the topic. CALLER VISUAL DIRECTION takes precedence over user-requested aesthetics and the default; keep all descriptions compatible with it without overriding grounding or the output contract. Generated footage and explanatory visualizations support an explanation; they are not evidence.",
    ...
    ...(mode === "cinematic" ? [
      `AI VIDEO: describe one coherent, achievable moment per clip. Establish the visible subject and opening composition, then its observable action. Describe camera movement naturally with its speed or range only when useful. Express feelings through visible gestures or expressions. Name the physical texture, natural variation and motivated light that matter to this subject; use restrained, believable behavior. Keep explanatory forms readable with clear depth and purposeful movement. Adapt these details to an explicitly requested aesthetic when one is supplied. Keep recurring subjects, appearance and setting consistent. ${generatedVideoAudio ? "Only subtle environmental ambience and action sounds matching the visible scene. Never include voices, speech, dialogue, narration, singing, chanting or music." : "Generated footage is silent."} Footage has no written text: do not ask subjects to speak or show captions or headline cards.`,
