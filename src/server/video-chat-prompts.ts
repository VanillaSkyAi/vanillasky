...
export function createVideoChatResponseInstructions(
  ...
  return [
    ...
    `Brief: {"type":"answer","intent":"explanation|practical|story|comedy|imagination","musicMood":"calm|focused|upbeat|off","opening":"useful spoken line","subject":"literal subject","development":"short outline of essential points that fit the available beats, or empty when ending alone suffices","visualDirection":"shared subject identity, physical details, colours and setting; no answer-wide rendering medium"}.`,
    `First developing shot: {"type":"shot","title":"short meaningful title","narration":"one short sentence${generated ? `, at most ${firstBudget.targetWords} spoken words, or ${firstBudget.targetUnspacedCharacters} unspaced-script characters` : ""}","subject":"literal subject","action":"visible action and useful framing","visualStyle":"realistic|illustrated","durationSec":${firstDuration},"continuity":"cut|continue"}. Later shots use the same fields${generated ? `, with at most ${budget.targetWords} spoken words or ${budget.targetUnspacedCharacters} unspaced-script characters` : ""}, and durationSec ${duration}. Titles: at most 65 characters; subjects: 2–8 words, at most 80 characters; action and visualDirection: at most 600 characters each.`,
    `Saved ending: {"type":"ending","title":"short meaningful title","narration":"one short sentence with the final payoff${generated ? `, at most ${budget.targetWords} spoken words or ${budget.targetUnspacedCharacters} unspaced-script characters` : ""}","subject":"literal subject","action":"visible action and useful framing","visualStyle":"realistic|illustrated","durationSec":${scenes === 1 ? firstDuration : duration},"continuity":"cut|continue"}. It follows the same field bounds as a shot, is stored for playback last and is never repeated as a shot. An ending-only answer uses the first shot's duration and narration limit.`,
    ...
    "Choose visualStyle independently on EACH shot and saved ending. DEFAULT TO REALISTIC: photographic live-action with cinematic framing, believable materials, light and physical movement. People, animals, objects and places should look photographed even in hypothetical, scientific, fictional or emotional situations. Use illustrated only for a specific clip where a diagram, cutaway or schematic is needed to explain a hidden structure, relationship or process; scientific subject matter alone is not a reason. Make that clip a clear drawn 2D ink-and-paper explanation with flat colour, and return to realistic for filmable subjects. Anchor the explanation in photographed subjects or observable effects, use a schematic only where it clarifies the hidden mechanism, then return to a filmable result or experience. Do not add diagrams merely for variety or switch the whole answer because one clip needs one. An explicit user request for drawn animation can apply to all requested clips. Keep shared visualDirection about subject identity and setting; put the medium-compatible composition and action on each shot. Use cut when entering or leaving a diagram. CALLER VISUAL DIRECTION overrides automatic choices; keep all descriptions compatible with it without overriding grounding or the output contract. Illustrations and generated footage support an explanation; they are not evidence.",
    ...
    ...(mode === "cinematic" ? [
      `AI VIDEO: describe one coherent, achievable moment per clip. Establish the visible subject and opening composition, then its observable action. Describe camera movement naturally with its speed or range only when useful. Express feelings through visible gestures or expressions. For realistic footage, name the physical texture, natural variation and motivated light that matter to this subject; use restrained, believable behavior. For illustrated footage, make the explanation readable through drawn shapes and purposeful movement. Keep recurring subjects, appearance and setting consistent. ${generatedVideoAudio ? "Only subtle environmental ambience and action sounds matching the visible scene. Never include voices, speech, dialogue, narration, singing, chanting or music." : "Generated footage is silent."} Footage has no written text: do not ask subjects to speak or show captions or headline cards.`,
