...
function App() {
  ...
  useEffect(() => {
    setRows([]);
    const detach = diagnostics.observeRecoveries(document);
    return () => {detach(); diagnostics.dispose();};
  }, [diagnostics]);
  const surface = rows.find(row => row.phase === "body rendered");
  const footage = rows.find(row => row.phase === "video decoded");
  return <><aside className="dev-toolbar">
    ...
    <span>{live ? "Host authorization and limits apply." : "Local waterfall + matching spoken fixtures; not live model output."} {surface && `Body rendered: ${surface.elapsedMs} ms. `}{footage && `Video decoded: ${footage.elapsedMs} ms.`}</span>
    <details><summary>Safe phase log ({rows.length})</summary><ol>{rows.map((row, index) => <li key={index}>{row.elapsedMs} ms · {row.phase}{row.reason && ` · ${row.reason}`}{row.durationMs !== undefined && ` · ${row.durationMs} ms duration`}{row.status && ` · HTTP ${row.status}`}{row.speechDurationSec !== undefined && ` · speech ${row.speechDurationSec.toFixed(2)} s`}{row.clipDurationSec !== undefined && ` · clip ${row.clipDurationSec.toFixed(2)} s`}{row.sceneDurationSec !== undefined && ` · scene ${row.sceneDurationSec.toFixed(2)} s`}{row.bufferedSeconds !== undefined && ` · buffered ${row.bufferedSeconds} s`}{row.repeatCount !== undefined && ` · repeats ${row.repeatCount}`}{row.recovered && " · chapter recovery"}</li>)}</ol></details>
