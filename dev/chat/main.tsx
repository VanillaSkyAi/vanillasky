...
function App() {
  ...
  return <><aside className="dev-toolbar">
    ...
    <details><summary>Safe phase log ({rows.length})</summary><ol>{rows.map((row, index) => <li key={index}>{row.elapsedMs} ms · {row.phase}{row.reason && ` · ${row.reason}`}{row.durationMs !== undefined && ` · ${row.durationMs} ms duration`}{row.status && ` · HTTP ${row.status}`}</li>)}</ol></details>
