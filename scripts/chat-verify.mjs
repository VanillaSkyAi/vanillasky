...
for (const gate of ["registry:check", "lint", "typecheck", "test", "acceptance:chat"]) run("npm", ["run", gate]);
...
for (const file of ["verify-public-api-surface.mjs", "verify-packed-package.mjs", "verify-onboarding.mjs", "verify-nextjs-onboarding.mjs"]) run(process.execPath, [`scripts/${file}`], env);
